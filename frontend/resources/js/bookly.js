            ...
            $.ajax({
                ...
                success     : function (response) {
                    if (response.success) {
                        ...
                        ;
						var mndate = gregorian_to_jalali(response.date_min[0],response.date_min[1]+1,response.date_min[2]);
                        var mxndate = gregorian_to_jalali(response.date_max[0],response.date_max[1]+1,response.date_max[2]);
                        var defval = $date_from.attr("data-value").split("-");
						
                        defval = gregorian_to_jalali(parseInt(defval[0]),parseInt(defval[1]),parseInt(defval[2]));
                        $date_from.val(defval[0]+"-"+ defval[1]+"-"+ defval[2]);

                        // Init Pickadate.
                        $date_from.datepicker({
	                        dateFormat: 'yy-mm-dd',
                            defaultDate:new JalaliDate(defval[0], defval[1], defval[2]),
                            minDate             : mndate[0]+"-"+mndate[1]+"-"+mndate[2],
                            maxDate             : mxndate[0]+"-"+mxndate[1]+"-"+mxndate[2] ,
                            ...
                            onSelect           : function(dateText, inst) {
                                var date =$( this ).datepicker("getDate").getGregorianDate() ;
                                var timestamp = date.valueOf();
                                $('.ab-week-day[value="' + (date.getDay() + 1) + '"]:not(:checked)', $container).attr('checked', true).trigger('change');
                                $(this).attr("data-value",date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate());
                            ...
                            if (stepServiceValidator()) {
                                ...
                                $.ajax({
                                    ...
                                    data : {
                                        ...
                                        form_id    : Options.form_id,
                                        chain      : chain,
                                        date_from  : $date_from.attr("data-value"),
                                        days       : days,
                ...
                success     : function (response) {
                    if (response.success) {
                        ...
                        var phone_number  = '',
                            ...
                            $errors       = $('.bookly-js-full-name-error, .bookly-js-user-phone-error, .bookly-js-user-email-error, .bookly-custom-field-error', $container),
                            $fields       = $('.bookly-js-full-name, .bookly-js-user-phone-input, .bookly-js-user-email, .bookly-custom-field', $container),
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.success) {
                                        ...
                                        $name_field.val(response.data.name).removeClass('bookly-error');
                ...
                success    : function (response) {
                    if (response.success) {
                        ...
                        $('.bookly-js-next-step', $container).on('click', function (e) {
                            ...
                            //console.log($('.bookly-payment[value=nextpay]',    $container).is(':checked'));
                            ...
                            if ($('.bookly-payment[value=local]', $container).is(':checked') || $(this).hasClass('bookly-js-coupon-payment')) {
                                ...
                                if (stripe && $form.find('#publishable_key').val()) {
                                    ...
                                    card_payment(data);
										...
										|| $('.bookly-payment[value=nextpay]',    $container).is(':checked')
										|| $('.bookly-payment[value=zarin]',      $container).is(':checked')
                                        || $('.bookly-payment[value=mellat]',     $container).is(':checked')

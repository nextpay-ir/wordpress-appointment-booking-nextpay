...
<div class="bookly-payment-nav">
	...
	<?php if ( $pay_nextpay ) : ?>
        <div class="bookly-box bookly-list">
            <label>
                <input type="radio" class="bookly-payment" name="payment-method-<?php echo $form_id ?>" value="nextpay"/>
                <span>پرداخت با درگاه امن نکست پی</span>
                <img style="height:30px" src="<?php echo plugins_url( 'frontend/resources/images/nextpay.png', \Bookly\Lib\Plugin::getMainFile() ) ?>" alt="nextpay" />
            </label>
            <?php if ( $payment['gateway'] == Bookly\Lib\Entities\Payment::TYPE_NEXTPAY && $payment['status'] == 'error' ) : ?>
    ...
    <?php if ( $pay_local ) : ?>
        <div class="bookly-box bookly-list bookly-test-payment">
            <label>
	...
	<?php if ( $pay_mellat ) : ?>
        <div class="bookly-box bookly-list">
            <label>
                ...
                <span><?php echo \Bookly\Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_label_pay_mellat' ) ?></span>
                <img style="height:30px" src="<?php echo plugins_url( 'frontend/resources/images/mellat.png', \Bookly\Lib\Plugin::getMainFile() ) ?>" alt="Mellat" />
            </label>
	...
	<?php if ( $pay_zarin ) : ?>
        <div class="bookly-box bookly-list">
            <label>
                ...
                <span><?php echo \Bookly\Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_label_pay_zarin' ) ?></span>
                <img style="height:30px" src="<?php echo plugins_url( 'frontend/resources/images/zarinpal.svg', \Bookly\Lib\Plugin::getMainFile() ) ?>" alt="zarinPal" />
            </label>
            <?php if ( $payment['gateway'] == Bookly\Lib\Entities\Payment::TYPE_ZARIN && $payment['status'] == 'error' ) : ?>
                <div class="bookly-label-error"><?php echo $payment['data'] ?></div>
            <?php endif ?>
        </div>
    <?php endif ?>
...
<?php if ( $pay_mellat ) : ?>
    <div class="bookly-gateway-buttons pay-mellat bookly-box bookly-nav-steps" style="display:none">
        <?php Bookly\Lib\Payment\Mellat::renderECForm( $form_id ) ?>
    </div>
<?php endif ?>
...
<?php if ( $pay_nextpay ) : ?>
    <div class="bookly-gateway-buttons pay-nextpay bookly-box bookly-nav-steps" style="display:none">
        <?php Bookly\Lib\Payment\Nextpay::renderECForm( $form_id ) ?>
    </div>
<?php endif ?>

<?php if ( $pay_zarin ) : ?>
    <div class="bookly-gateway-buttons pay-zarin bookly-box bookly-nav-steps" style="display:none">
        <?php Bookly\Lib\Payment\Zarinpal::renderECForm( $form_id ) ?>
    </div>
<?php endif ?>

...
class SMS
{
    ...
    public function sendSms( $phone_number, $message, $type_id = null )
    {
        //if ( $this->token ) {
            ...
            if ( $data['phone'] != '' ) {
                if(get_option( 'bookly_sms_beferest' )=="ec"){
                    $link = new \SoapClient(get_option( 'bookly_sms_beferest_url' ),array('encoding' => 'UTF-8'));
                    $data = array(
                    'username' => get_option( 'bookly_sms_beferest_username' ),
                    'password' => get_option( 'bookly_sms_beferest_password' ),
                    'from' => get_option( 'bookly_sms_beferest_number' ),
                    'to'=>$this->normalizePhoneNumber( $phone_number ),
                    'text' => $message,
                    'isflash'=>'',
                    );
                    try{
            	   $res = $link->SendSimpleSMS2($data);
                   $response= new  \stdClass ;
                   $response->gateway_status = $res->SendSmsResult==0 && $res->status==0?1:-1;
